...
export default function DaftarTransaksi() {
  ...
  const fetchTransaksi = useCallback(
    async (isRefresh = false) => {
      if (!isLoggedIn || !token) {
        setError('Silakan masuk untuk melihat transaksi');
        setLoading(false);
        setRefreshing(false);
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(res.errors || 'Gagal mengambil data transaksi');
        }

        if (res.data?.transaksi) {
          let filteredTransaksi = res.data.transaksi;
          if (startDate && endDate) {
            filteredTransaksi = filteredTransaksi.filter((trx) => {
    ...
    [isLoggedIn, token, startDate, endDate]
  );
          ...
          <View style={styles.filterSection}>
            ...
            <View style={styles.filterRow}>
              <View style={styles.datePickerContainer}>
                ...
                >
                  <Text style={styles.dateButtonText}>
                    {startDate ? format(startDate, 'dd MMM yyyy', { locale: id }) : 'Pilih Tanggal Awal'}
                  </Text>
                </TouchableOpacity>
                {showStartPicker && (
                  <DateTimePicker
              ...
              <View style={styles.datePickerContainer}>
                ...
                >
                  <Text style={styles.dateButtonText}>
                    {endDate ? format(endDate, 'dd MMM yyyy', { locale: id }) : 'Pilih Tanggal Akhir'}
                  </Text>
                </TouchableOpacity>
                {showEndPicker && (
                  <DateTimePicker
